...
<section class="wallet-sect transaction-sect bg-light py-4 clearfix">
	...
	<div class="block mb-3">
			<p class="mb-1 semi">Sattalment Transection</p>	
	
		    <?php
 $namesesduser =$_SESSION['finalplayer'];
    $sql="SELECT * FROM `sattlement_request` WHERE player_phone='$namesesduser' ORDER BY id DESC";
    $run=mysqli_query($conn,$sql);

    if(mysqli_num_rows($run)<1)
    {
       echo '<p class="alert alert-danger">Sattalment Transection Data Not Found</p>';
    }
else{
    ...
    <div class="box box-style icon-fill-none transectionhistory	">
		    	<div class="d-flex align-items-center">
					...
					<div class="">
						<p class="mb-0 semi"><?php echo $data['playername']; ?></p>
						 
						<div class="text-muted bal"><?php echo $data['date']; ?>
							<?php  if($data['status']=="1"){
				            echo '<span class="text-warning"style="font-weight:bold;">Pending..</span>';
				}
				elseif($data['status']=="2"){
				            echo '<span class="text-success" style="font-weight:bold;">Transfered</span>';
				}
				elseif($data['status']=="3"){
				            echo '<span class="text-primary" style="font-weight:bold;">Canceled</span>';
				}
    ...
    ;?>
				...
				<?php   echo '<span class="text-danger">-';
				; ?>
				
				<?php echo $data['playeramount']; ?></span></div>
